...
const OPTIONS = [
  'Rarely, it\u2019s unpredictable',
  'Only a few days a month',
  'Never when it matters most',
  'It changes day by day',
  'It clears up, then crashes',
  'Honestly, almost never',
  'It feels total guesswork',
];
...
export default function OnboardingPage6() {
  ...
  const [selected, setSelected] = useState<string[]>(answers['q2_cooperation'] || []);
  ...
  const handleNext = () => {
    if (selected.length > 0) {
      setAnswer('q2_cooperation', selected);
      navigate('/onboarding/page-6b');
    }
  };
      ...
      <View style={styles.content}>
        <View style={styles.titleContainer}>
          <View style={styles.titleFlow}>
            {`But how often does your skin actually`.split(' ').map((word, i) => (
              <TitleText key={`plain-${i}`} style={styles.titleOverride}>{word}{' '}</TitleText>
            ))}
            <View style={styles.highlightWrapper}>
              <View style={styles.highlightStripe} />
              <TitleText style={styles.titleOverride}>cooperate?</TitleText>
            </View>
          </View>
        </View>
