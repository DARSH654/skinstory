...
const OPTIONS = [
  'Stop checking every mirror',
  'Finally feeling like myself',
  'Drop the product guesswork',
  'Show up completely unbothered',
  'Stop hiding under makeup',
  'Wake up feeling relieved',
...
export default function OnboardingPage5() {
  ...
  const { setGoodDayFeeling, setAnswer, answers, goodDayFeeling } = useOnboardingStore.getState();
  const [selected, setSelected] = useState<string[]>(answers['q1_goodDayFeeling'] || goodDayFeeling || []);
  ...
  const handleNext = () => {
    if (enabled) {
      setGoodDayFeeling(selected);
      setAnswer('q1_goodDayFeeling', selected);
      navigate('/onboarding/page-6');
    }
  };
      ...
      <View style={styles.content}>
        <View style={styles.titleContainer}>
          <View style={styles.titleFlow}>
            {`${displayName}, what's the best part of a`.split(' ').map((word, i) => (
              <TitleText key={`plain-${i}`} style={styles.titleOverride}>{word}{' '}</TitleText>
            ))}
            <HighlightPhrase
              words={['perfect ', 'skin ', 'day?']}
